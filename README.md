...
Biological insights were given based on the main Principal Components (PC1 and PC2), which explained the most variance (~63%) of the top 10 components.
      ...
      1) Together, these two Principal Components gave a clear message that most tumors sit somewhere in the space of 
      shape irregularity or distortion (PC1) and complexity (PC2).
      
      2) Other PCs (PC3 - PC10) are not very significant in determining malignancy, considering their individual variation 
      is very low. For example, PC3 is less than 10% and continues to decrease until PC10, which is less than 2%
      
      3) Our data suggest that the primary biological difference between a benign and a malignant tumor is how distorted or
      irregular their structure is. In other words, malignant tumors tend to have irregular margins.
      
      4) The most diagnostic information in the dataset sits in a single hidden pattern (PC1)
